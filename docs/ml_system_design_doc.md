...
1. Технические метрики (см. [Результаты обучения модели](./training_summary.md)):
 ...
 - Precision ≥ 90%
 - Recall ≥ 90%
 - Среднее время обработки одного изображения ≤ 3 секунды
...
3. Image procession: Выполняет предварительную обработку изображения (удаление фона, повышение контрастности), запускает модели анализа, сохраняет результаты в виде изображений с аннотациями.
4. Model Service: Управляет загрузкой и выбором моделей, реализует детекцию проблем кожи с использованием предобученных моделей (YOLO, CLIP).
...
## 4.3. Требования к работе системы
 ...
 - Задержка: не более 3 секунд на анализ изображения.
